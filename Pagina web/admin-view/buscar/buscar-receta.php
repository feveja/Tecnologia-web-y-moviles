...
<html>
	<head>
		...
		<title>Buscar receta: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Busqueda de receta por nombre de paciente</h3>
	
	    <form action="buscar-resultados.php" method="post">
			<input type="hidden" name="sender" value="receta"/>
			<p>Nombre de paciente: <select name="codigoreceta">
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigo, nombre, apellido, run from receta, paciente where receta.runpaciente=paciente.run") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option label='".$linea['run']." ".ucfirst($linea['nombre'])." ".ucfirst($linea['apellido'])."'>" . $linea['codigo'] . "</option>";
					}
				?>
			</select></p>
