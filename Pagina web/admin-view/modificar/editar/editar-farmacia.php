...
<html>
	<head>
		...
		<title>Editar farmacia: admin</title>
	...
	<div id="contenido">
		<h3>Para editar una farmacia, rellene los datos del formulario a continuacion</h3>
	    ...
	    <p>Codigo farmacia:
				...
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigo from farmacia") or die('Consulta fallida');
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='farmacia'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'farmacia';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
