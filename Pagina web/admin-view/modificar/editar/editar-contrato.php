...
<html>
	<head>
		...
		<title>Editar contrato: admin</title>
	...
	<div id="contenido">
		<h3>Para editar un contrato, rellene los datos del formulario a continuacion</h3>
	
	    <form action="editar-submit.php" method="post">
		<input type="hidden" name="sender" value="contrato"/>
			<p>Codigo contrato:
				<select name="codigocontrato">
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigocontrato from contrato") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . $linea['codigocontrato'] . "</option>";
					}
				?>
				</select>
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='contrato'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'contrato';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
		...
		<table>
			...
			<?php 
				$consulta = pg_exec("select * from mostrar_contratos()") or die("Consulta fallida");
