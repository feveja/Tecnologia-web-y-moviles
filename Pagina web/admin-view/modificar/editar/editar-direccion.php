...
<html>
	<head>
		...
		<title>Editar direccion: admin</title>
	...
	<div id="contenido">
		<h3>Para editar una direccion, rellene los datos del formulario a continuacion</h3>
	
	    <form action="editar-submit.php" method="post">
		<input type="hidden" name="sender" value="direccion"/>
	    <p>Codigo postal:
				<select name="codigo">
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigopostal from direccion") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . $linea['codigopostal'] . "</option>";
					}
				?>
				</select>
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='direccion'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'direccion';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
		...
		<table>
			...
			<?php 
				$consulta = pg_exec("select * from direccion") or die("Consulta fallida");
				while ($contenido = pg_fetch_assoc($consulta)){
					echo "<tr> 
						<td>".$contenido['codigopostal']."</td>
						<td>".$contenido['calle']."</td>
						<td>".$contenido['numero']."</td>
						<td>".$contenido['comuna']."</td>
						</tr>";
				}
			?>
		</table>
