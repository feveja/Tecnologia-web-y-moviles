...
<html>
	<head>
		...
		<title>Editar fecha de supervision: admin</title>
	...
	<div id="contenido">
		<h3>Para editar una fecha de supervision, rellene los datos del formulario a continuacion</h3>
	
	    <form action="editar-submit.php" method="post">
		<input type="hidden" name="sender" value="fechasup"/>
	    <p>RUN Supervisor:
				<select name="runsupervisor">
				<option>---</option>
				<?php 
					$consulta = pg_query("select runsupervisor from fechasup") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . $linea['runsupervisor'] . "</option>";
					}
				?>
				</select>
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='fechasup'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'fechasup';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
		...
		<table>
			<tr>
				...
				<th>
					Fecha inicio de supervision
				</th>
				<th>
					Fecha termino de supervision
				</th>
			</tr>
			<?php 
				$consulta = pg_exec("select * from fechasup") or die("Consulta fallida");
				while ($contenido = pg_fetch_assoc($consulta)){
					echo "<tr> 
						<td>".$contenido['runsupervisor']."</td>
						<td>".$contenido['fechainiciosupervision']."</td>
						<td>".$contenido['fechaterminosupervision']."</td>
						</tr>";
				}
			?>
		</table>
