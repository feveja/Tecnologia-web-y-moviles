...
<html>
	<head>
		...
		<title>Editar supervisor: admin</title>
	...
	<div id="contenido">
	    ...
	    <p>RUN Supervisor:
				...
				<option>---</option>
				<?php 
					$consulta = pg_query("select run from supervisor") or die('Consulta fallida');
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='supervisor'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'supervisor';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
		...
		<table>
			...
			<?php 
				$consulta = pg_exec("select * from mostrar_supervisor()") or die("Consulta fallida");
