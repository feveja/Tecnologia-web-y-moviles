...
<html>
	<head>
		...
		<title>Editar receta: admin</title>
	...
	<div id="contenido">
	    ...
	    <p>Codigo receta:
				...
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigo from receta") or die('Consulta fallida');
				...
				<select name="atributo">
					<option>---</option>
				<?php 
					$consulta = pg_query("select column_name from information_schema.columns as tabla where tabla.table_name='receta'
										except
										select column_name
										from information_schema.table_constraints as tc
										join information_schema.key_column_usage as ku on tc.constraint_name = ku.constraint_name
										and ku.table_name = 'receta';") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
		...
		<table>
				...
				<?php 
				$consulta = pg_exec("select * from mostrar_receta()") or die("Consulta fallida");
