...
<html>
	<head>
		...
		<title>Borrar compra: admin</title>
	...
	<div id="contenido">
		<h3>Para borrar una compra, rellene los datos del formulario a continuacion</h3>
	
	    <form action="borrar-submit.php" method="post">
		<input type="hidden" name="sender" value="comprar"/>
			<p>Nombre comercial del medicamento:
				<select name="runpaciente" readonly>
					<option><?php echo $_POST['runpaciente']?></option>
				</select>
				<select name="nombrecomermed">
				<option>---</option>
				<?php 
					$consulta = pg_query("select nombrecomermed from mostrar_comprar() where runpaciente='".$_POST['runpaciente']."'") or die('Consulta fallida');
		...
		<table>
				...
				<?php 
				$consulta = pg_exec("select * from mostrar_comprar()") or die("Consulta fallida");
