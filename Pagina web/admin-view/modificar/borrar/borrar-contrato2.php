...
<html>
	<head>
		...
		<title>Borrar contrato: admin</title>
	...
	<div id="contenido">
		<h3>Para borrar un contrato, rellene los datos del formulario a continuacion</h3>
	
	    <form action="borrar-submit.php" method="post">
		<input type="hidden" name="sender" value="contrato"/>
			<p>Codigo contrato:
				<select name="codigocontrato">
				<option><?php echo $_POST['codigocontrato']?></option>
				</select>
				<select name="runsupervisor">
				<option>---</option>
				<?php 
					$consulta = pg_query("select runsupervisor from mostrar_contratos() where codigocontrato=".$_POST['codigocontrato']."") or die('Consulta fallida');
