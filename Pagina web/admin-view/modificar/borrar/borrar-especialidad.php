...
<html>
	<head>
		...
		<title>Borrar especialidad: admin</title>
	...
	<div id="contenido">
		<h3>Para borrar una especialidad, rellene los datos del formulario a continuacion</h3>
	
	    <form action="borrar-submit.php" method="post">
		<input type="hidden" name="sender" value="especialidad"/>
			<p>Nombre de especialidad:
		...
		<table>
			...
			<?php 
				$consulta = pg_exec("select * from mostrar_especialidades()") or die("Consulta fallida");
				while ($contenido = pg_fetch_assoc($consulta)){
					echo "<tr> 
						<td>".ucfirst($contenido['mostrar_especialidades'])."</td>
						</tr>";
				}
			?>
		</table>
