...
<html>
	<head>
		...
		<title>Borrar existencias: admin</title>
	...
	<div id="contenido">
		<h3>Para borrar una existencia, rellene los datos del formulario a continuacion</h3>
	
	    <form action="borrar-submit.php" method="post">
		<input type="hidden" name="sender" value="existeen"/>
			<p>Nombre comercial medicamento:
				<select name="nombrecomercialmedicamento">
				<option><?php echo $_POST['nombrecomercialmedicamento']?></option>
				</select>
				<select name="codigofarmacia">
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigofarmacia from mostrar_existeen() where nombrecomermed='".strtolower($_POST['nombrecomercialmedicamento'])."'") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . $linea['codigofarmacia'] . "</option>";
					}
				?>
				</select>
			</p>
		...
		<table>
			...
			<?php 
				$consulta = pg_exec("select * from mostrar_existeen()") or die("Consulta fallida");
				while ($contenido = pg_fetch_assoc($consulta)){
					...
					if ($contenido['fechainiciooferta'] == null) {
						...
						echo "<td>".$contenido['fechainiciooferta']."</td>";
					}
					if ($contenido['fechafinoferta'] == null) {
						echo "<td>No hay registros</td>";
					} else {
						echo "<td>".$contenido['fechafinoferta']."</td>";
					}
					echo "</tr>";
				}
			?>
