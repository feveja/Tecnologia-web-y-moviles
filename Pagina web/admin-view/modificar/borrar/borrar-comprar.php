...
<html>
	...
	<div id="contenido">
	    ...
	    <form action="borrar-comprar2.php" method="post">
			<p>RUN Paciente:
				<select name="runpaciente">
				<option>---</option>
				<?php 
					$consulta = pg_query("select runpaciente from mostrar_comprar()") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . $linea['runpaciente'] . "</option>";
					}
				?>
				</select>
			</p>
