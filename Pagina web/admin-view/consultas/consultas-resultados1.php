...
<html>
	<head>
		...
		<?php
			...
			$consulta = pg_exec("select * from consulta1()") or die("Consulta fallida");
		?>
	...
	<div id="contenido">
		...
		<table>
			<tr>
				<th>RUN paciente</th><th>Nombre paciente</th><th>RUN Medico</th><th>Nombre medico</th><th>Fecha receta</th><th>Nombre farmacia</th><th>Nombre medicamento</th><th>Precio medicamento</th>
			</tr>
			<?php 
				while ($contenido = pg_fetch_assoc($consulta)){
					echo "<tr> 
						<td>".$contenido['run_paciente']."</td>
						<td>".ucfirst($contenido['nombre_paciente'])."</td>
						<td>".$contenido['run_medico']."</td>
						<td>".ucfirst($contenido['nombre_medico'])."</td>
						<td>".date("d-m-Y", strtotime($contenido['fecha_receta']))."</td>
						<td>".ucwords($contenido['nombre_farmacia'])."</td>
						<td>".ucfirst($contenido['nombre_medicamento'])."</td>
						<td>".$contenido['precio_medicamento']."</td>
						</tr>";
				}
			?>
		</table>
