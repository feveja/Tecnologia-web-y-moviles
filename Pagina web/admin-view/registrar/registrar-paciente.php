...
<html>
	<head>
		...
		<title>Registrar paciente: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar un nuevo paciente, ingrese los datos del formulario a continuacion</h3>
	
	<form action="registrar-submit.php" method="post">
			<input type="hidden" name="sender" value="paciente"/>
		<p>RUN: <input type="text" name="run"> Fecha de nacimiento: <input type="date" name="fechanac"> Correo: <input type="text" name="correo"></p>
		...
		<select name="prevision">
				...
				<?php 
					...
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . ucwords($linea['nombre']) . "</option>";
					}
				?>
			</select>
			Medico a cargo:
			<select name="runmedico">
				<option>---</option>
				<?php 
		...
		<p>Codigo telefono: 
			<?php
					$total = pg_fetch_row(pg_exec("select max(codigo) from telefono"));
				?> 
			<input type="number" value=<?php echo $total[0]+1 ?> name="codigotelefono" disabled> 
