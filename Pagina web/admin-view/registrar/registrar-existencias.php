...
<html>
	<head>
		...
		<title>Registrar existencias: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar existencias de medicamentos, ingrese los datos a continuacion</h3>
	
	    <form action="registrar-submit.php" method="post">
			<input type="hidden" name="sender" value="existencias"/>
			<p>Nombre comercial de medicamento: 
			<select name="nombrecomermed">
				...
				<?php 
					...
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . ucwords($linea['nombrecomercial']) . "</option>";
					}
				?>
			</select>
			Farmacia: 
			<select name="codigofarmacia">
				<option>---</option>
				<?php 
					$consulta = pg_query("select codigo, nombre from farmacia") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option label='".$linea['codigo']." ".ucwords($linea['nombre'])."'>" . $linea['codigo'] . "</option>";
					}
				?>
			</select></p>
			<p>Precio medicamento: <input type="number" name="preciomed"> Cantidad medicamento: <input type="number" name="cantidadmed"></p>
