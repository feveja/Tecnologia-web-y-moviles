...
<html>
	<head>
		...
		<title>Registrar medicamento: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar un nuevo medicamento, ingrese los datos del formulario a continuacion</h3>
	
	    <form action="registrar-submit.php" method="post">
			<input type="hidden" name="sender" value="medicamento"/>
			<p>Nombre comercial medicamento: <input type="text" name="nombrecomermed"> Formula: <input type="text" name="formula"> 
			Empresa farmaceutica asociada: 
			<select name="codigoempresafarmaceutica">
				...
				<?php 
					...
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option label='".$linea['codigos']." ".ucwords($linea['nombre'])."'>" . $linea['codigo'] . "</option>";
					}
				?>
			</select></p>
