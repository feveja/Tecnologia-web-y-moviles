...
<html>
	<head>
		...
		<title>Registrar farmacia: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar una nueva farmacia, ingrese los datos del formulario a continuacion</h3>
	
	    <form action="registrar-submit.php" method="post">
			<input type="hidden" name="sender" value="farmacia"/>
			<p>Codigo farmacia:
			<?php
					$total = pg_fetch_row(pg_exec("select max(codigo) from farmacia"));
				?> 
			<input type="number" value=<?php echo $total[0]+1 ?> name="codigofarmacia" disabled> 

			Nombre farmacia:
			<input type="text" name="nombrefarmacia"></p>
			<p>Telefono farmacia: <input type="number" name="telefonofarmacia"> Direccion farmacia: <input type="text" name="direccionfarmacia"></p>
