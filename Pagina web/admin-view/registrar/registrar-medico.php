...
<html>
	<head>
		...
		<title>Registrar medico(a): admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar un(a) nuevo(a) medico(a), ingrese los datos del formulario a continuacion</h3>
	
	    <form action="registrar-submit.php" method="post">
			<input type="hidden" name="sender" value="medico"/>
            <p>RUN medico: <input type="text" name="runmedico"> Fecha de nacimiento: <input type="date" name="fechanacimiento" max=<?php echo date("Y-m-d") ?>> A&ntilde;os ejerciendo: <input type="number" name="aniosejerciendo"></p>
            <p>Nombre: <input type="text" name="nombre"> Apellido: <input type="text" name="apellido"> Imagen: <input type="text" name="imagen"> Especialidad: 
			<select name="especialidad">
				<option>---</option>
				<?php 
					$consulta = pg_query("select nombre from especialidad") or die('Consulta fallida');
