...
<html>
	<head>
		...
		<title>Registrar empresa farmaceutica: admin</title>
		<?php 
	...
	<div id="contenido">
		<h3>Para registrar una nueva empresa farmaceutica, ingrese los datos del formulario a continuacion</h3>
	    ...
	    <form action="registrar-submit.php" method="post">
			...
			<?php
					$total = pg_fetch_row(pg_exec("select max(codigo) from empresafarmaceutica"));
				?> 
			<input type="number" value=<?php echo $total[0]+1 ?> name="codigoempresafarmaceutica" disabled> 
			Nombre empresa farmaceutica:
			<input type="text" name="nombreempresafarmaceutica">
			Telefono:
			<input type="number" name="telefonoempresafarmaceutica"></p>
