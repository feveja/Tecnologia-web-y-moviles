...
<html>
	<head>
		...
		<script language="Javascript">
			function correcto_revertir() {
				alert("Se ha revertido la oferta");
			}
			function correcto_ofertar() {
				alert("Se ha ofertado el medicamento");
			}
			function error() {
				alert("Ha ocurrido un error");
			}
			function redireccionar_ofertas() {
				window.location.href="home-admin-ofertar-medicamento.php";
			}
			function redireccionar_revertir() {
				window.location.href="home-admin-revertir-oferta.php";
			}
		</script>
	</head>
	<body>
	...
	<div id="contenido">
        ...
        <?php
            switch ($identificador) {
                case 'revertir':
					$ejecucion = pg_exec("select RevertirOferta('".strtolower($_POST['nombrecomermed'])."')");
					if ($ejecucion == false) {
						echo "<script>error()</script>";
						echo "<script>redireccionar_revertir()</script>";
					} else {
						echo "<script>correcto_revertir()</script>";
						echo "<script>redireccionar_revertir()</script>";
					}
                    break;
                case 'ofertar':
                    $ejecucion = pg_exec("select PonerOferta('".strtolower($_POST['nombrecomercialmed'])."',".$_POST['preciooferta'].",'".$_POST['fechainiciooferta']."','".$_POST['fechaterminooferta']."')");
					if ($ejecucion == false) {
						echo "<script>error()</script>";
						echo "<script>redireccionar_ofertas()</script>";
					} else {
						echo "<script>correcto_ofertar()</script>";
						echo "<script>redireccionar_ofertas()</script>";
						// echo "Se ha oferta el medicamento ".$_POST['nombrecomercialmed']." a un precio de ".$_POST['preciooferta'];
					} 
                    break;
            }
        ?>
