...
<html>
	...
	<div id="contenido">
        ...
        <h4>Para revertir la oferta no vigente de un articulo seleccione el medicamento a continuacion</h4>
        <form action="home-admin-ofertas-submit.php" method="post">
			<input type="hidden" name="sender" value="revertir"/>
			<p>Nombre comercial medicamento: <select name="nombrecomermed">
				<option>---</option>
				<?php 
					$consulta = pg_query("select nombrecomermed from existeen where fechafinoferta<=current_date") or die('Consulta fallida');
					while ($linea = pg_fetch_array($consulta)) {
						echo "<option>" . ucfirst($linea['nombrecomermed']) . "</option>";
					}
				?>
			</select></p>
            <p><input type="submit" value="Revertir"> <input type="reset" value="Limpiar formulario"></p>
	    </form>
    </div>
	</body>
